...
- Components (such as task cards) will subscribe *surgically* only to their specific task state using exact ID/hash selectors (e.g., `useTaskStore(state => state.tasks[taskHash])`).
